...
#import "WXWBaseConfigurableCell.h"
#import "WXWUIUtils.h"
...
@interface WXWBaseConfigurableCell(Private)

- (float) shadowMargin;

@end

@implementation WXWBaseConfigurableCell (Private)
...
@interface WXWConfigurableTableViewCellBackground : UIView

@property (nonatomic, assign) WXWBaseConfigurableCell *cell;
...
@implementation WXWConfigurableTableViewCellBackground
@synthesize cell;
@synthesize behavior;
...
@implementation WXWBaseConfigurableCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
  ...
  if (self) {
    ...
    WXWConfigurableTableViewCellBackground *bg = [[WXWConfigurableTableViewCellBackground alloc] initWithFrame:self.frame
                                                                                    behavior:CellBackgroundBehaviorNormal];
    bg.cell = self;
    self.backgroundView = bg;
    [bg release];
    
    bg = [[WXWConfigurableTableViewCellBackground alloc] initWithFrame:self.frame
...
{
  ConfigurableTableViewCellPosition position = [WXWBaseConfigurableCell positionForTableView:tableView indexPath:indexPath];
  return [WXWBaseConfigurableCell neededHeightForPosition:position tableStyle:tableView.style];
}
...
- (void) prepareForTableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath
{
  ...
  self.position = [WXWBaseConfigurableCell positionForTableView:tableView indexPath:indexPath];
}
  ...
  if ([keyPath isEqualToString:@"frame"])
  {
    ...
    if (diffY != 0)
    {
      CGRect rect = CGRectMake(originalFrame.origin.x+shadowMargin,
                               ...
                               originalFrame.size.height- CONTENTVIEW_MARGIN*2 - [WXWBaseConfigurableCell neededHeightForPosition:self.position tableStyle:_tableViewStyle]);
