  ...
  if (nil == infoGradientBackgroundView) {
    ...
    switch (msgType) {
        case ERROR_TY:
            ...
            infoBarTopColor = COLOR(53,53,53);//COLOR(59, 151, 238);//COLOR(194, 15, 15);
            infoBarBottomColor = COLOR(115,115,115);//COLOR(40, 118, 208);//COLOR(145, 14, 14);
    ...
    [((BaseAppDelegate*)APP_DELEGATE).window addSubview:infoGradientBackgroundView];
    
    [((BaseAppDelegate*)APP_DELEGATE).window bringSubviewToFront:infoGradientBackgroundView];
  }
...
+ (void)addInfoLabelToWindow:(MessageType)msgType
  ...
  if (nil == infoBackgroundView) {
    ...
    infoBackgroundView.center = ((BaseAppDelegate*)APP_DELEGATE).window.center;
  ...
  [((BaseAppDelegate*)APP_DELEGATE).window addSubview:infoBackgroundView];
  
  [((BaseAppDelegate*)APP_DELEGATE).window bringSubviewToFront:infoBackgroundView];
}
...
+ (void)closeAsyncLoadingView {
  ...
  if (asyncLoadingBackgroundView && asyncLoadingLabel && operaFacebookImageView) {
    ...
    asyncLoadingBackgroundView.frame = CGRectMake(0, ((BaseAppDelegate*)APP_DELEGATE).window.frame.size.height, ((BaseAppDelegate*)APP_DELEGATE).window.frame.size.width, ASYNC_LOADING_VIEW_HEIGHT);
    [UIView commitAnimations];
  }
}
...
+ (void)showAsyncLoadingView:(NSString *)message toBeBlockedView:(UIView *)toBeBlockedView {
  ...
  if (nil == asyncLoadingBackgroundView) {
    asyncLoadingBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, ((BaseAppDelegate*)APP_DELEGATE).window.frame.size.height, ((BaseAppDelegate*)APP_DELEGATE).window.frame.size.width, ASYNC_LOADING_VIEW_HEIGHT)];
    asyncLoadingBackgroundView.backgroundColor = [UIColor blackColor];
    asyncLoadingBackgroundView.alpha = 0.7f;
    [((BaseAppDelegate*)APP_DELEGATE).window addSubview:asyncLoadingBackgroundView];
  }
  ...
  CGSize size = [asyncLoadingLabel.text sizeWithFont:asyncLoadingLabel.font
                                   constrainedToSize:CGSizeMake(200, CGFLOAT_MAX)
                                       lineBreakMode:UILineBreakModeWordWrap];
  asyncLoadingLabel.frame = CGRectMake(((BaseAppDelegate*)APP_DELEGATE).window.frame.size.width - MARGIN * 2 - size.width, ASYNC_LOADING_VIEW_HEIGHT/2 - size.height/2, size.width, size.height);
  ...
  asyncLoadingBackgroundView.frame = CGRectMake(0, ((BaseAppDelegate*)APP_DELEGATE).window.frame.size.height - ASYNC_LOADING_VIEW_HEIGHT, ((BaseAppDelegate*)APP_DELEGATE).window.frame.size.width, ASYNC_LOADING_VIEW_HEIGHT);
...
+ (void)drawActivityView:(NSString *)text {
	...
	if (nil == activityBackgroundView) {
		...
		activityBackgroundView.center = ((BaseAppDelegate*)APP_DELEGATE).window.center;
	}
  ...
  if (text && [text length] > 0) {
    ...
    if (nil == loadingLabel) {
      ...
      loadingLabel.center = ((BaseAppDelegate*)APP_DELEGATE).window.center;
  ...
  activityBackgroundView.transform = CGAffineTransformRotate(CGAffineTransformIdentity, RADIANS(0));
	...
	[((BaseAppDelegate*)APP_DELEGATE).window addSubview:activityBackgroundView];
	[((BaseAppDelegate*)APP_DELEGATE).window bringSubviewToFront:activityBackgroundView];
...
+ (UIToolbar *)createKeyboardToolbarWithTarget:(id)target
                                nextItemAction:(SEL)nextItemAction
                            hideKeyboardAction:(SEL)hideKeyboardAction {
  
  if (nil == nextItemAction && nil == hideKeyboardAction) {
    return nil;
  }
  
  UIToolbar *toolbar = [[[UIToolbar alloc]initWithFrame:CGRectMake(0, 0, APP_WINDOW.frame.size.width, 40)] autorelease];
  
  [toolbar setBarStyle:UIBarStyleBlack];
  
  UIBarButtonItem *leftButton = nil;
  UIBarButtonItem *midButton = [[[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:nil action:nil] autorelease];

  if (nextItemAction) {
    leftButton = [[[UIBarButtonItem alloc]initWithTitle:LocaleStringForKey(NSNextItemTitle, nil)
                                                  style:UIBarButtonItemStyleDone
                                                 target:target
                                                 action:nextItemAction] autorelease];    
  } else {
    leftButton = [[[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:nil action:nil] autorelease];
  }
  
  UIBarButtonItem *hideButton = [[[UIBarButtonItem alloc]initWithTitle:LocaleStringForKey(NSHideKeyboardTitle, nil)
                                                  style:UIBarButtonItemStyleDone
                                                 target:target
                                                 action:hideKeyboardAction] autorelease];
  
  NSArray *buttonsArray = @[leftButton, midButton, hideButton];
