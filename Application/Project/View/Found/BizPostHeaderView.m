...
@interface BizPostHeaderView()
@property (nonatomic, copy) NSString *content;
@end

@implementation BizPostHeaderView
...
- (void)changeTextValue:(NSString *)textValue tag:(NSString *)tag type:(int)type {
    [_textView setText:textValue];
    [_editorDelegate textChanged:textValue];
    
    [_tagsField setText:tag];
    
    switch (type) {
        case SUPPLY_POST_TY:
        {
            [self selectSupply:nil];
        }
            break;
        case DEMAND_POST_TY:
        {
            [self selectDemand:nil];
        }
            break;
    }
